    ...
    for batch_idx, (inputs_src1, _, domainIDs_src, indexes_src) in enumerate(source_loader):
        ...
        for i in range(len(args.cluster_num_list)):
            if args.in_domain:
                ...
                loss_cdm1 += criterion(logits_src1[i], (logits_mem_src1 / tau).softmax(1).detach())

                logits_mem_tar1 = model.top_layer_tgt[i](model.lemniscate_t.memory[indexes_tar])
                loss_cdm2 += criterion(logits_tar1[i], (logits_mem_tar1 / tau).softmax(1).detach())
            if args.cross_domain:
                ...
                if args.cross_domain_loss == 'l1':
                    loss_cdm3 += args.lambda_cross_domain*(
                        (outputs_src_cls_tgt - outputs_tgt_cls_tgt).abs().sum(1).mean())
                    loss_cdm4 += args.lambda_cross_domain * (
        ...
        loss_cdm1 /= len(args.cluster_num_list)
        loss_cdm2 /= len(args.cluster_num_list)
        loss_cdm3 /= len(args.cluster_num_list)
        loss_cdm4 /= len(args.cluster_num_list)
    

        loss_cdm = (loss_cdm1 + loss_cdm2) + (loss_cdm3 + loss_cdm4)
        loss_cdm.backward()
        ...
        if batch_idx % 50 == 0:
            print(f"Step [{batch_idx}/{len(source_loader)}] loss_cdm1: {loss_cdm1} loss_cdm2: {loss_cdm2} loss_cdm3: {loss_cdm3} loss_cdm4: {loss_cdm4}")
